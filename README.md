...
- 귀무가설 : 상관계수가 0이다.(상관관게가 없다)

- 대립가설 : 상관계수는 0이 아니다.(상관관계가 있다)

분산을 검정하는 역할의 함수 var.test(x,y) : x,y의 분산이 같은지 확인 (등분산?)

결정계수는 추정한 선형 모델이 주어진 데이터에 적합한 정도를 재는 척도로서 설명력(R-squared) - 1에 가까울 수록 설명력이 좋음

    lm(종속변수(결과)~독립변수(원인),데이터) 함수를 사용하여 선형회귀 모델을 생성할 수 있다.
...
우선 상관분석을 통해 유의미 하지 않은것을 제외하고, 다중회귀분석을 통해 덜 유의미한것을 제외함. -step, vif등 다중공선성을 통해 문제있는 독립변수 추릴 수 
