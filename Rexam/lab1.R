...
which(nums<=50)
which(nums==max(nums))#which.max(nums)
which(nums==min(nums))
...
myFriend[which(max(score)==score)]
myFriend[which(min(score)==score)]
myFriend[which(score>10)]

#문제6
count=sample(1:100,7)
week.korname=c("일요일","월요일","화요일","수요일","목요일","금요일","토요일")
vec=paste(week.korname,count,sep=":")
vec
week.korname[max(vec)==vec]
week.korname[min(vec)==vec]
